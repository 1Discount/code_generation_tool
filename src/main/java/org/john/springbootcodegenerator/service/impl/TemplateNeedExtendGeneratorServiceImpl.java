...
public class TemplateNeedExtendGeneratorServiceImpl implements TemplateGeneratorService {
	...
	private Map<String, Object> replaceMap(String tableName, String classDescription) {
		...
		map.put("instanceName", FormatNameUtlis.formatNameLowerCase(tableColumnsService.getClassName(tableName)));
	...
	private String getPackgePath(String name) {
		if (TemplateCommon.entity.equals(name)) {
			return extendsSettings.getBasePath();
		} else if (TemplateCommon.vo.equals(name)) {
			return codeGeneratorConfig.getPackgePath() + "." + TemplateCommon.vo;
	...
	private String getTemplateFileName(String name) {
		if (TemplateCommon.entity.equals(name)) {
			...
			return codeGeneratorConfig.getTemplateBasePath() + TemplateCommon.voTemplateFileName;
		} else if (TemplateCommon.dao.equals(name)) {
	...
	private String getWriteFilePath(String name) {
		if (TemplateCommon.entity.equals(name)) {
			...
			if (StringUtils.isNotEmpty(codeGeneratorConfig.getWriteFileBaseVoPath())) {
				return codeGeneratorConfig.getWriteFileBaseVoPath();
			}
			return codeGeneratorConfig.getWriteFileBasePath() + TemplateCommon.vo;
		} else if (TemplateCommon.dao.equals(name)) {
	...
	private String getWriteFileName(String name, String tableName) {
		if (TemplateCommon.entity.equals(name)) {
			return tableColumnsService.getEntityClassName(tableName) + TemplateCommon.javaSuffix;
		} else if (TemplateCommon.vo.equals(name)) {
			return tableColumnsService.getClassName(tableName) + TemplateCommon.javaVOSuffix;
		} else if (TemplateCommon.dao.equals(name)) {
	...
	private void fileWrite(String name, String tableName, String fileName) {
		if (TemplateCommon.entity.equals(name)) {
			...
			FileUtlis.wirteContent(getWriteFilePath(TemplateCommon.vo),
				getWriteFileName(TemplateCommon.vo, tableName), fileName);
		} else if (TemplateCommon.dao.equals(name)) {
	...
	public void createEntityTemplate(String tableName, String classDescription) {
		...
		map.put("className", tableColumnsService.getEntityClassName(tableName));
		...
		if (extendsSettings.isExtendsBaseEntity()) {
			String extendBaseEntityPath = extendsSettings.getBaseEntityPath();
			if (StringUtils.isNotEmpty(extendBaseEntityPath)) {
				map.put("extendBaseEntityPath", extendBaseEntityPath);
				String[] paths = extendBaseEntityPath.split("\\.");
				map.put("extendBaseEntity",paths[paths.length - 1] );
			}
		}
	...
	public void createVOTemplate(String tableName, String classDescription) {
		...
		map.put("alias", tableColumnsService.getAliasName(tableName));
		map.put("table", tableName);
		map.put("entityData", tableColumnsService.getVOData(tableName));
		if (extendsSettings.isExtendsBaseEntity()) {
			String extendBaseEntityPath = extendsSettings.getBaseEntityPath();
			if (StringUtils.isNotEmpty(extendBaseEntityPath)) {
				map.put("extendBaseEntityPath", extendBaseEntityPath);
	...
	public void createDaoTemplate(String tableName, String classDescription) {
		...
		if (extendsSettings.isExtendsBaseDao()) {
			...
			String baseEntityPath = extendsSettings.getBaseEntityPath();
			if (StringUtils.isNotEmpty(baseEntityPath)) {
				map.put("extendBaseEntityPath", baseEntityPath);
				String[] paths = baseEntityPath.split("\\.");
				map.put("extendBaseEntity",paths[paths.length - 1] );
			}
		}
		map.put("entityClassName", tableColumnsService.getEntityClassName(tableName));
		fileName = ReplaceUtlis.replace(fileName, map);
	...
	public void createServiceTemplate(String tableName, String classDescription) {
		...
		map.put("entityPackgePath", getPackgePath(TemplateCommon.entity));
		map.put("voPackgePath", getPackgePath(TemplateCommon.vo));
		map.put("entityClassName", tableColumnsService.getEntityClassName(tableName));
